...
| RF-22 | M | La ficha muestra rótulo, dirección, municipio, horario declarado y los seis combustibles de Precios con su precio. |
...
| RF-76 | M | La cadena territorial del catálogo del ministerio es la clave canónica para emparejamientos, identificadores y slugs, y no se modifica. El proyecto no traduce ni elige entre denominaciones: nombres dobles como `ARABA/ALAVA` y `CASTELLÓN / CASTELLÓ` se conservan, y `GIRONA` no se sustituye por otra denominación. Solo al pintar se deriva un nombre visible mediante dos transformaciones deterministas y sin mantenimiento recurrente: una tabla fija con los nombres bien escritos de las 19 comunidades autónomas y una regla común para provincias y municipios que mueve al principio el artículo final `(La)`, `(A)`, `(El)`, `(Els)`, `(Les)`, `(Illes)`, `(Las)` o `(Los)`. La tabla debe cubrir las 19 claves ministeriales y el build falla indicando cualquier comunidad desconocida. Ninguna transformación del nombre visible cambia claves, emparejamientos, identificadores, slugs ni URL. |
...
| RF-86 | M | El rótulo se muestra verbatim; la dirección se pasa a caja de título; municipio y provincia usan el nombre visible definido por RF-76. |
...
| RF-119 | M | Un municipio conserva su página mientras le quede al menos una estación visible, si alcanzó `MINIMO_ESTACIONES_MUNICIPIO` en algún momento de la ventana histórica. Con cero estaciones visibles, o sin haber alcanzado nunca el mínimo, la URL devuelve 404 real. El sitemap y los enlaces internos siguen el mismo criterio. |
...
| RF-94 | M | El precio que acompaña a cada municipio, tanto en el bloque de municipios de una página de zona como en el de vecinos de una página de municipio, es el del combustible seleccionado. Si ninguna estación de ese municipio lo vende, se muestra «Aquí no hay {combustible}»; nunca se muestra el precio de otro. |
...
| RF-122 | El GLP usa la escala territorial de color cuando se comparan estaciones que venden GLP, calculada solo con sus precios y bajo las condiciones de ADR-0025. En el panel que coloca los seis combustibles uno debajo de otro se presenta sin banda y con una nota que explica que su precio por litro no es comparable con el de los demás por su mayor consumo en volumen. Queda fuera de las comparaciones entre combustibles, las páginas editoriales y las imágenes de compartición. |
...
| RF-125 | Cada sección declara y respeta su propio conjunto de combustibles: los seis en Precios y en las comparaciones entre estaciones del mismo combustible; cinco —incluido gasóleo B y excluido GLP— en editoriales y comparaciones entre combustibles; cuatro con histórico disponible en Evolución; y gasolina 95 y diésel en las imágenes de compartición. Ver [ADR-0028](adr/0028-cada-seccion-tiene-su-propio-conjunto-de-combustibles.md). |
...
| RF-127 | Las ausencias no se intercambian: estación que no vende, «no vende»; ámbito sin estaciones vendedoras, «Aquí no hay {combustible}»; observación diaria ausente, «sin dato»; escala suprimida, el texto común de ADR-0025; combustible fuera de una sección, explicación común de ADR-0028; histórico insuficiente para el periodo, «Aún no hay histórico suficiente para comparar…»; y muestra insuficiente para un ranking, «No hay muestra suficiente para entrar en este ranking». Un fallo de carga del histórico queda fuera de estos estados y conserva su mensaje de error. |
...
- **V2-18.3.** Si ninguna estación pública de la provincia vende el combustible,
  la pastilla dice «Aquí no hay {combustible} · 0 estaciones» y no muestra un precio
