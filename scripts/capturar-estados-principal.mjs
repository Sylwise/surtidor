...
const [base = 'http://127.0.0.1:4173', directorioSalida = '/tmp', puerto = '9225'] = process.argv.slice(2);
...
async function caso(nombre, ancho, alto, expresion) {
  ...
  await comando('browsingContext.navigate', { context, url: `${base}/asturias/`, wait: 'complete' });
  ...
  const salida = `${directorioSalida}/principal-${nombre}.png`;
...
await caso('hoy-desktop', 1440, 1024, `(()=>{const mapa=document.querySelector('#mapa');const antes=mapa.getBoundingClientRect().width;const b=document.querySelector('#boton-hoy');b.focus();b.click();return JSON.stringify({abierto:!document.querySelector('#panel-hoy').hidden,foco:document.activeElement?.getAttribute('href'),mapaNoEncoge:antes===mapa.getBoundingClientRect().width})})()`);
